...
class Usuarios {
    ...
    private function readRecord($position) {
        ...
        // Posicionar en el registro específico
        fseek($handle, $position * $this->recordSize);
        // Leer el registro completo
        $record = fread($handle, $this->recordSize);
        fclose($handle);
        if (strlen($record) < $this->recordSize) return false; // No hay suficientes datos para un registro completo
    ...
    public function findByLogin($login) {
        ...
        foreach ($users as $index => $user) {
            if ($user['UsrLogin'] === $login) return ['index' => $index, 'data' => $user];
        }
        return false;
    }
    ...
    public function verifyCredentials($login, $password) {
        ...
        if ($user['UsrPassword'] === $password && $user['UsrStatus'] === '1') return $user;
        return false;
    }
    // Crear un nuevo usuario
    public function create($userData) {
        // Verificar si el login ya existe
        if ($this->findByLogin($userData['UsrLogin']) !== false) return false; 
